...
import { isOfferOrder } from '../../tools/driverOffer'
...
function OrderCard({
  ...
  const getStatusText = () => {
    if (offerOrder) return t(TRANSLATION.CLIENT_OFFER_TITLE)
    if (order.b_voting) return t(TRANSLATION.VOTER)
    return ''
  }

  const getStatusTextColor = () => {
    if (offerOrder) return '#FF2400'
    ...
    >
      ...
      {offerOrder && (
        <div className="status-card__offer-summary">
          <div className="status-card__offer-badge">{t(TRANSLATION.CLIENT_OFFER_TITLE)}</div>
