        ...
        <form>
          <fieldset>
            ...
            {order?.drivers?.map(item => {
              ...
              const driverOffer = (item.c_options as any)?.driver_offer || (item.c_options as any)?.offer || {}
              const offerPrice = (item.c_options as any)?.driver_offer_price ?? driverOffer.price ?? driverOffer.performers_price ?? item.c_options?.performers_price
              const offerEta = (item.c_options as any)?.driver_offer_eta ?? driverOffer.eta ?? driverOffer.pickup_time ?? driverOffer.arrival_time
              const offerSeats = (item.c_options as any)?.driver_offer_free_seats ?? driverOffer.free_seats ?? driverOffer.freeSeats ?? driverOffer.seats
              const offerComment = (item.c_options as any)?.driver_offer_comment ?? driverOffer.comment ?? driverOffer.driver_comment
                        ...
                        {item.u_registration.format(dateFormatDate)}</p> */}
                          <p className="candidate__header-replies">
                            ...
                            {offerPrice !== undefined &&
                              <span className="candidate__header-subinfo-item">{t(TRANSLATION.PRICE_PERFORMER)}: {offerPrice} {CURRENCY.SIGN}</span>
                            }
                            {offerEta &&
                              <span className="candidate__header-subinfo-item">{t(TRANSLATION.DRIVER_OFFER_ETA)}: {offerEta}</span>
                            }
                            {offerSeats &&
                              <span className="candidate__header-subinfo-item">{t(TRANSLATION.DRIVER_OFFER_SEATS)}: {offerSeats}</span>
                            }
                            {offerComment &&
                              <span className="candidate__header-subinfo-item">{t(TRANSLATION.DRIVER_OFFER_COMMENT)}: {offerComment}</span>
                            }
                          </p>
                        </div>
                        <div
