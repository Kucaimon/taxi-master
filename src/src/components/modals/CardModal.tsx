...
function CardModalContent({
  ...
  useEffect(() => {
    if (!active || (!order?.b_voting && !isOfferMode && !currentDriverOffer))
      return

    const interval = window.setInterval(() => setNow(Date.now()), 1000)
    return () => window.clearInterval(interval)
  }, [active, order?.b_voting, isOfferMode, currentDriverOffer])
  ...
  const handleSubmit = () => orderMutation(async() => {
    if (shouldUseOfferFlow) {
      ...
      const storedOffer = saveStoredDriverOffer(orderId, user?.u_id, offerPayload)
  ...
  const getButtons = () => {
    ...
    if (shouldUseOfferFlow) {
      if (currentDriverOffer && !currentOfferExpired && currentDriverOffer.status !== 'expired')
        return <>
          <div className="driver-offer-sent-card">
        ...
        {isOfferMode &&
          <div className="driver-offer-info">
            ...
            <div className="driver-offer-info__row">
              ...
              <b>{getSafeAddressText(currentLanguage, address?.shortAddress, address?.address, order?.b_start_address)} → {getSafeAddressText(currentLanguage, destinationAddress?.shortAddress, destinationAddress?.address, order?.b_destination_address)}</b>
            </div>
