...
import { IDriverOfferPayload } from '../tools/driverOffer'
    ...
    .then(car => {
      ...
      addToFormData(formData, {
        ...
        data: JSON.stringify({
          c_id: car.c_id,
          c_payment_way: EPaymentWays.Cash,
          c_options: { performers_price: performersPrice },
        }),
      })
...
const _sendOrderOffer = (
  { formData }: IApiMethodArguments,
  id: IOrder['b_id'],
  offer: IDriverOfferPayload,
) => {
  const userID = userSelectors.user(store.getState())?.u_id
  if (!userID)
    return Promise.reject({ message: t(TRANSLATION.WRONG_USER_ROLE) })

  // OFFER строим поверх уже существующего Truck/Candidate-механизма:
  // SetPerformer + performer=0 добавляет водителя как кандидата, а клиент
  // выбирает исполнителя через старый список откликнувшихся водителей.
  return getUserCar(userID as string)
    .then(car => {
      if (!car) return Promise.reject({ message: t(TRANSLATION.NOT_LINKED_CAR), error: 'used_car_not_found' })

      const validKeys = SITE_CONSTANTS.C_OPTIONS_VALID_KEYS || {}
      const canSendOption = (key: string) => Boolean((validKeys as Record<string, boolean>)[key])
      const cOptions: Record<string, unknown> = {
        performers_price: offer.price,
      }

      // В Truck/gruzvill старый backend строго проверяет c_options_valid_keys.
      // Поэтому обязательную цену отправляем через старый performers_price,
      // а новые поля OFFER добавляем только если они разрешены в константах
      // текущего конфига. Это не ломает старый candidate-flow.
      if (canSendOption('driver_offer_price')) cOptions.driver_offer_price = offer.price
      if (canSendOption('driver_offer_eta')) cOptions.driver_offer_eta = offer.eta
      if (canSendOption('driver_offer_free_seats')) cOptions.driver_offer_free_seats = offer.freeSeats
      if (canSendOption('driver_offer_comment')) cOptions.driver_offer_comment = offer.comment || ''
      if (canSendOption('driver_offer_mode')) cOptions.driver_offer_mode = 'OFFER'
      if (canSendOption('driver_offer')) {
        cOptions.driver_offer = {
          price: offer.price,
          eta: offer.eta,
          free_seats: offer.freeSeats,
      ...
      return axios.post(`${Config.API_URL}/drive/get/${id}`, formData)
        .then(res => res.data)
        .then(res => res.status === 'error' || res.code === 'error' ? Promise.reject(res) : res)
    })
}
