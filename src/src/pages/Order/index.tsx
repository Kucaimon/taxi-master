...
function formatOfferDistance(order: any) {
  ...
  const normalizedMeters = meters > 1000 ? meters : meters * 1000
  return `~${Math.round(normalizedMeters / 1000)} км`
}

function formatOfferDuration(order: any) {
...
function getOfferDesiredPrice(order: any) {
  return getNumericValue(order?.b_options?.customer_price, order?.b_price_estimate)
}
...
const Order: React.FC<IProps> = ({
  ...
  useEffect(() => {
    if (!order?.b_voting && !isOfferMode && !currentDriverOffer)
      return

    const interval = window.setInterval(() => setNow(Date.now()), 1000)
    return () => window.clearInterval(interval)
  }, [order?.b_voting, isOfferMode, currentDriverOffer])
  ...
  const handleSubmit = () => {
    ...
    if (shouldUseOfferFlow) {
      ...
      const offerPayload: IDriverOfferPayload = {
        ...
        eta: values.offerEta,
        freeSeats: values.offerSeats,
        comment: values.offerComment,
      }
      API.sendOrderOffer(id, offerPayload)
        .then(() => {
          const storedOffer = saveStoredDriverOffer(id, user?.u_id, offerPayload)
          ...
          setMessageModal({
            isOpen: true,
            status: EStatuses.Success,
            message: t(TRANSLATION.DRIVER_OFFER_SENT_STATUS),
          })
        })
        .catch(error => {
          console.error(error)
  ...
  const getButtons = () => {
    ...
    if (shouldUseOfferFlow) {
      if (currentDriverOffer && !currentOfferExpired && currentDriverOffer.status !== 'expired') return <>
        <div className="driver-offer-sent-card order__driver-offer-sent-card">
          <b>{t(TRANSLATION.DRIVER_OFFER_SENT)}</b>
          <span>{t(TRANSLATION.DRIVER_OFFER_OWN_PRICE)}: {formatOfferPrice(currentDriverOffer.price)}</span>
          <span>{t(TRANSLATION.DRIVER_OFFER_OWN_ETA)}: {currentDriverOffer.eta}</span>
          <span>{t(TRANSLATION.DRIVER_OFFER_SEATS)}: {currentDriverOffer.freeSeats}</span>
          <span>{t(TRANSLATION.DRIVER_OFFER_STATUS)}: {getOfferStatusText(currentDriverOffer.status)}</span>
        </div>
        <Button
      ...
      if (offerFormOpen) return <>
        ...
        <Input
          inputProps={{
            ...
            placeholder: 'Буду через 15 минут',
          }}
          error={errors?.offerEta?.message}
          label={t(TRANSLATION.DRIVER_OFFER_ETA)}
          oneline
        />
        <Input
          inputProps={{
            ...register('offerSeats', { required: t(TRANSLATION.REQUIRED_FIELD), min: 1, valueAsNumber: true }),
            type: 'number',
            min: 1,
          }}
          error={errors?.offerSeats?.message}
          label={t(TRANSLATION.DRIVER_OFFER_SEATS)}
          oneline
        />
        <Input
          ...
          inputProps={{
            ...
            placeholder: 'Есть кондиционер, большой багажник, еду напрямую',
          }}
          ...
          <form onSubmit={formHandleSubmit(handleSubmit)}>
            ...
            {isOfferMode &&
              <div className="driver-offer-info order__driver-offer-info">
                <div className="driver-offer-info__title">{t(TRANSLATION.CLIENT_OFFER_TITLE)}</div>
                <div className="driver-offer-info__row">
                  <span>{t(TRANSLATION.DRIVER_OFFER_ROUTE)}</span>
                  <b>{start?.shortAddress || start?.address || order.b_start_address} → {destination?.shortAddress || destination?.address || order.b_destination_address}</b>
                </div>
                {formatOfferDistance(order) &&
                  <div className="driver-offer-info__row">
                    <span>{t(TRANSLATION.DRIVER_OFFER_DISTANCE)}</span>
                    <b>{formatOfferDistance(order)}</b>
                  </div>
                }
                {formatOfferDuration(order) &&
                  <div className="driver-offer-info__row">
                    <span>{t(TRANSLATION.DRIVER_OFFER_TRAVEL_TIME)}</span>
                    <b>{formatOfferDuration(order)}</b>
                  </div>
                }
                <div className="driver-offer-info__row">
                  <span>{t(TRANSLATION.DRIVER_OFFER_PASSENGERS)}</span>
                  <b>{order.b_passengers_count || 0}</b>
                </div>
                <div className="driver-offer-info__row">
                  <span>{t(TRANSLATION.DRIVER_OFFER_LUGGAGE)}</span>
                  <b>{order.b_luggage_count || 0}</b>
                </div>
                {offerDesiredPrice !== undefined &&
                  <div className="driver-offer-info__row">
                    <span>{t(TRANSLATION.DRIVER_OFFER_DESIRED_PRICE)}</span>
                    <b>{formatOfferPrice(offerDesiredPrice)}</b>
                  </div>
                }
                {offerCommentText &&
                  <div className="driver-offer-info__comment">
                    <span>{t(TRANSLATION.DRIVER_OFFER_CLIENT_COMMENT)}</span>
                    <p>{offerCommentText}</p>
                  </div>
                }
                ...
                <div className="driver-offer-info__hint">{t(TRANSLATION.DRIVER_OFFER_NO_FAKE_COMPETITION)}</div>
              </div>
            }
