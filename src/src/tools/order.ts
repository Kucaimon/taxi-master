...
function normalizeDriverResponseMode(value: unknown) {
  const number = Number(value)
  return Number.isFinite(number) ? number : value
}

export function candidateMode(order?: IOrder): boolean {
  switch (normalizeDriverResponseMode(SITE_CONSTANTS.DRIVER_RESPONSE_MODE)) {
    ...
    case EDriverResponseModes.ByOrder:
      for (const id of order?.b_comments ?? [])
        switch (normalizeDriverResponseMode(SITE_CONSTANTS.BOOKING_COMMENTS[id]?.responseMode)) {
          case EDriverResponseModes.Performer:
            return false
          case EDriverResponseModes.Candidate:
            return true
